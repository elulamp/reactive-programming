...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    def handleRequests = handleRequest(handler, listener)_
  ...
  def handleRequest(handler: Request => Response, listener: Listener)(ct: CancellationToken): Unit = {
    if (ct.nonCancelled) {
      val f = listener.nextRequest()

      f onComplete {
        case Success((request, exchange)) => Future(respond(exchange, ct, handler(request))); handleRequest(handler, listener)(ct)
        case Failure(t) => handleRequest(handler, listener)(ct)
      }
    }
  }
