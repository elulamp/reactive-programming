...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  def verify(probe: TestProbe, ops: Seq[Operation], expected: Seq[OperationReply]): Unit = {
    val topNode = system.actorOf(Props[BinaryTreeSet], "TreeSet")
  ...
  test("foo") {
    ...
    topNode ! Insert(testActor, id = 1, 71)
    topNode ! Insert(testActor, id = 3, 67)
    topNode ! Remove(testActor, id = 11, 67)
    topNode ! GC
    topNode ! Contains(testActor, id = 5, 49)
    topNode ! Contains(testActor, id = 6, 76)
    topNode ! Insert(testActor, id = 7, 76)
    topNode ! Insert(testActor, id = 8, 87)
    topNode ! Contains(testActor, id = 9, 87)


    var msgs = List.empty[OperationReply]

    val repliesUnsorted = for (i <- 1 to 7) yield try {
      msgs = msgs :+ expectMsgType[OperationReply]
    } catch {
