...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs map { Success(_) } onErrorReturn { error => Failure(error) }
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs.map(value => { requestMethod(value).recovered }).concat
