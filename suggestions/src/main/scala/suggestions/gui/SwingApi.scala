...
trait SwingApi {
  ...
  implicit class TextFieldOps(val field: TextField) {
    ...
    def textValues: Observable[String] = {
      ...
      Observable.create { o =>
        val s = Subscription {
          field.unsubscribe {
            case ValueChanged(e) => o.onCompleted()
          }

        }

        field.subscribe {
          case ValueChanged(e) => if (!s.isUnsubscribed) o.onNext(e.text)
        }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = {
      ...
      Observable.create { o =>
        val s = Subscription {
          button.unsubscribe {
            case ButtonClicked(e) => o.onCompleted()
          }
        }

        button.subscribe {
          case ButtonClicked(btn) => if (!s.isUnsubscribed) o.onNext(btn)
        }
